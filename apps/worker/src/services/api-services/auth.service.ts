...
export class AuthService implements IAuthService {
  ...
  async login(request: LoginRequest): Promise<LoginResponse> {
    ...
    await this.authDataService.storeRefreshToken(user.id, refreshToken, expiresAt);
  ...
  async register(request: RegisterRequest): Promise<UserInfo> {
    ...
    const existingUser = await this.authDataService.getUserByEmail(request.email);
    if (existingUser) {
  ...
  async refreshToken(request: RefreshTokenRequest): Promise<LoginResponse> {
    const tokenData = await this.authDataService.getRefreshToken(request.refreshToken);
    ...
    await this.authDataService.storeRefreshToken(user.id, newRefreshToken, expiresAt);
  ...
  async changePassword(userId: string, request: ChangePasswordRequest): Promise<void> {
    ...
    const currentPasswordValid = await this.verifyPassword(request.currentPassword, userId);
  ...
  async resetPassword(request: ResetPasswordRequest): Promise<void> {
    ...
    await this.authDataService.storePasswordResetToken(request.email, resetToken, expiresAt);
  ...
  async confirmResetPassword(request: ResetPasswordConfirmRequest): Promise<void> {
    const tokenData = await this.authDataService.getPasswordResetToken(request.token);
    if (!tokenData || tokenData.expiresAt < new Date()) {
