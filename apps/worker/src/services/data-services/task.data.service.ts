...
export class KVTaskDataService implements TaskDataService {
  ...
  async getTasksByProject(projectId: string, query?: PaginationQuery): Promise<PagedResult<TaskDto>> {
    try {
      ...
      let taskIds = taskListResult.keys.map(key => key.name.split(':').pop()!);
      ...
      if (taskIds.length === 0 && this.apiService) {
        try {
          ...
          console.warn(`API fallback failed for project ${projectId} tasks:`, apiError);
        }
      }
  ...
  private async clearProjectTasksCache(projectId: string): Promise<void> {
    try {
      const taskListResult = await this.kvService.list(`task:project:${projectId}:`);
      const deletePromises = taskListResult.keys.map(key => this.kvService.delete(key.name));
      await Promise.all(deletePromises);
    } catch (error) {
      console.warn(`Failed to clear project tasks cache for ${projectId}:`, error);
    }
  }
}