...
export class TasksHttpService {
    async getTasksAsync(ctx: Context): Promise<Response> {
        try {
            ...
            const result = await ctx.var.taskService.getTasksByProject(projectId, pagination);
    ...
    async getTasksByScheduleAsync(ctx: Context): Promise<Response> {
        try {
            ...
            const result = await ctx.var.taskService.getTasksBySchedule(scheduleId, pagination);
    ...
    async getTasksByParentAsync(ctx: Context): Promise<Response> {
        try {
            ...
            const childTasks = await ctx.var.taskService.getTasksByParent(parentTaskId);
            ...
            return ctx.json({
                success: false,
                message: error instanceof Error ? error.message : 'Failed to get child tasks'
            }, 500);
        }
    }
    ...
    async getWeatherSensitiveTasksAsync(ctx: Context): Promise<Response> {
        try {
            const projectId = ctx.req.param('projectId');
            const weatherSensitiveTasks = await ctx.var.taskService.getWeatherSensitiveTasks(projectId);
            ...
            return ctx.json({
                success: false,
                message: error instanceof Error ? error.message : 'Failed to get weather sensitive tasks'
            }, 500);
        }
    }
    ...
    async createTaskAsync(ctx: Context): Promise<Response> {
        try {
            ...
            if (task.startDate && task.endDate) {
                ...
                task.durationInDays = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
    ...
    async updateTaskAsync(ctx: Context): Promise<Response> {
        try {
            ...
            if (updatedTask.startDate && updatedTask.endDate) {
                ...
                updatedTask.durationInDays = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
                updatedTask.isOverdue = end < new Date() && updatedTask.percentComplete < 100;
            }

            updatedTask.isInProgress = !!updatedTask.actualStartDate && !updatedTask.actualEndDate;
