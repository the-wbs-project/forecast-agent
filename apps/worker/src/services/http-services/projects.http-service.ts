...
export class ProjectsHttpService {
    async getProjectsAsync(ctx: Context): Promise<Response> {
        try {
            ...
            if (search) {
                result = await ctx.var.projectService.searchProjects(search, organizationId, pagination);
            } else {
                result = await ctx.var.projectService.getProjectsByOrganization(organizationId, pagination);
            }
    ...
    async updateProjectAsync(ctx: Context): Promise<Response> {
        try {
            ...
            const existingProject = await ctx.var.projectService.getProjectById(projectId);
            if (!existingProject) {
                return ctx.json({
                    success: false,
                    message: 'Project not found'
            ...
            const updatedProject = await ctx.var.projectService.getProjectById(projectId);
