...
export class WeatherHttpService {
  async getForecastAsync(ctx: Context): Promise<Response> {
    try {
      ...
      if (!forecast) {
        forecast = await ctx.var.weatherApiService.getForecast(latitude, longitude, days);
        // Cache for 1 hour
        await ctx.var.weatherService.storeForecastData(cacheKey, forecast, 3600);
      }
  ...
  async getAnalysesByProjectAsync(ctx: Context): Promise<Response> {
    try {
      ...
      const result = await ctx.var.weatherService.getAnalysesByProject(projectId, pagination);
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to get weather analyses'
      }, 500);
    }
  }
  ...
  async getAnalysisByIdAsync(ctx: Context): Promise<Response> {
    try {
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to get weather analysis'
      }, 500);
    }
  }
  ...
  async getHighRiskAnalysesAsync(ctx: Context): Promise<Response> {
    try {
      ...
      const result = await ctx.var.weatherService.getHighRiskAnalyses(organizationId, pagination);
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to get high risk analyses'
      }, 500);
    }
  }
  ...
  async createAnalysisAsync(ctx: Context): Promise<Response> {
    try {
      ...
      const forecast = await ctx.var.weatherApiService.getForecast(request.latitude, request.longitude, Math.min(days, 14));
      ...
      const analysis: WeatherRiskAnalysisDto = {
        ...
        riskLevel,
        riskScore,
        weatherCondition: request.weatherCondition || forecast[0]?.condition || 'Unknown',
        impactDescription: `Risk analysis based on ${days} days of weather forecast data`,
        recommendedActions: riskFactors.length > 0 ? [
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to create weather analysis'
      }, 500);
    }
  }
  ...
  async generateAnalysisAsync(ctx: Context): Promise<Response> {
    try {
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to generate weather analysis'
      }, 500);
    }
  }
  ...
  async updateAnalysisAsync(ctx: Context): Promise<Response> {
    try {
      ...
      const existingAnalysis = await ctx.var.weatherService.getAnalysisById(analysisId);
      if (!existingAnalysis) {
        return ctx.json({
          success: false,
          message: 'Weather analysis not found'
      ...
      const updatedAnalysis = await ctx.var.weatherService.getAnalysisById(analysisId);
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to update weather analysis'
      }, 500);
    }
  }
  ...
  async deleteAnalysisAsync(ctx: Context): Promise<Response> {
    try {
      ...
      return ctx.json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to delete weather analysis'
      }, 500);
    }
  }
}