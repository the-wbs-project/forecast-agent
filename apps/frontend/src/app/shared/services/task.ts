...
export class TaskService {
  ...
  async getTasksByProject(projectId: string, pageNumber = 1, pageSize = 50): Promise<PagedResult<Task>> {
    ...
    try {
      ...
      const response = await firstValueFrom(this.http.get<ApiResponse<PagedResult<Task>>>(
  ...
  async updateTask(id: string, task: Partial<Task>): Promise<Task> {
    ...
    try {
      const response = await this.http.put<ApiResponse<Task>>(
        `${this.apiUrl}/${id}`,
        task
      ).toPromise();
  ...
  async getTaskRisks(taskId: string): Promise<TaskRiskDetail[]> {
    ...
    try {
      const response = await this.http.get<ApiResponse<TaskRiskDetail[]>>(
        `${this.apiUrl}/${taskId}/risks`
      ).toPromise();
