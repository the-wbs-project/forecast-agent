...
export class ProjectDetailComponent implements OnInit {
  ...
  async loadProjectData(projectId: string) {
    ...
    try {
      ...
      this.error.set(error instanceof Error ? error.message : 'Failed to load project data');
    } finally {
  ...
  async deleteProject() {
    if (confirm('Are you sure you want to delete this project? This action cannot be undone.')) {
      try {
        ...
        this.error.set(error instanceof Error ? error.message : 'Failed to delete project');
      }
    }
  }
}